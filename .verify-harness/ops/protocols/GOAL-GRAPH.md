...
6. If decisions were made in a Lavish artifact, capture them in the tracker with `./verify-harness lavish tracker capture --issue <id> --artifact <html-file>` before implementation starts. Add `--decisions <file>` when decisions are recorded separately.
...
Use `./verify-harness orchestration next`, `orchestration prompt`, and `orchestration launch-spec` for hierarchy and task launch. Use `goals status`, `goals verify`, and `goals start-prompt` for goal-graph-specific local evidence. Do not maintain a second role or lifecycle taxonomy in this protocol.
...
./verify-harness goals verify <goal-id>
./verify-harness goals start-prompt <goal-id>
./verify-harness orchestration next
