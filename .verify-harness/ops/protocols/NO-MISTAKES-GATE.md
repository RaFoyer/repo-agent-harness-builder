...
- CLI wrappers: `./verify-harness no-mistakes status` and `./verify-harness no-mistakes setup [--fork-url <url>] [--agent <agent>]`
...
2. Run `./verify-harness no-mistakes status`.
3. If setup is not initialized, ask for approval, then run `./verify-harness no-mistakes setup`.
...
5. Run `./verify-harness verify` and any task-specific checks.
6. Commit the branch.
...
- Use `./verify-harness no-mistakes status` for value-safe summaries.
...
- Treat `./verify-harness no-mistakes setup` as a mutating operation because it
...
- If no-mistakes setup changes, update both `scripts/setup-no-mistakes.sh` and
  `./verify-harness no-mistakes setup`.
