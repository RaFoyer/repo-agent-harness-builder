...
If this linear graph uses delegated tasks, configure its nodes in the selected named private orchestration instance with `GOAL-GRAPH` as a governing protocol and `repository-merge` as the completion profile. Role, title, parentage, lifecycle, trust, and authority come from `AGENT-ORCHESTRATION.md` rather than this document.
