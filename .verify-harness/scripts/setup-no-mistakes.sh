...
print_unavailable() {
  ...
  echo "  \"Run ./verify-harness no-mistakes status for a value-safe summary\""
}
...
if ! tmp_status="$(mktemp 2>/dev/null)"; then
  ...
  echo "  initialized: false"
  echo "help[1]:"
  echo "  \"Install mktemp or run ./verify-harness no-mistakes status for a value-safe summary\""
  exit 1
fi
