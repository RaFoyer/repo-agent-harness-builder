...
const VERIFY_STEPS = [
  ...
  { name: "orchestration report --example", run: (io) => runOrchestration(["report", "--example"], io) },
  { name: "orchestration reconcile --example", run: (io) => runOrchestration(["reconcile", "--example"], io) },
...
export async function runVerify(argv, io) {
  ...
  if (hasFlag(argv, "--help") || hasFlag(argv, "-h")) {
    io.stdout("Usage: ./verify-harness verify [--dry-run]");
