...
| Project orchestration | inactive | `ops/protocols/AGENT-ORCHESTRATION.md`, inactive `ops/orchestration.example.json`, named private instances, `docs/templates/orchestration/`, `./{{CLI_NAME}} orchestration status --example`, and `orchestration liveness --example`; initialize with `orchestration init <name>` and activate only after scope, owner, root materialization, trust policy, control-loop policy, authority envelopes, budgets, and evidence profiles are configured |
