...
| Project orchestration | inactive | `ops/protocols/AGENT-ORCHESTRATION.md`, inactive `ops/orchestration.example.json`, named private instances, `docs/templates/orchestration/`, `./{{CLI_NAME}} orchestration status --example`; initialize with `orchestration init <name>` and activate only after scope, owner, root materialization, trust policy, authority envelopes, budgets, and evidence profiles are configured |
