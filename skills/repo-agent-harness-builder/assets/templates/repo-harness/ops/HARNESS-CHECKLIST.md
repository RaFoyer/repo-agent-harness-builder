...
| Codex-native Firstmate adapter | inactive | `ops/protocols/CODEX-NATIVE-FIRSTMATE.md`, `.agents/skills/codex-native-firstmate/SKILL.md`, `.codex/config.firstmate.example.toml`, `./{{CLI_NAME}} orchestration adapter-status`; capability is resident per repository but activates only after a human configures repo-local scope, one Firstmate/Boss task identity, task-creation grant, trust, authority, budgets, completion profiles, adapter, base/worktree policy, Browser/GitHub integration, heartbeat, retention/archive policy, and binding assurance; no global project registry required |
