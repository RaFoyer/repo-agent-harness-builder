...
| Codex-native Firstmate adapter | inactive | `ops/protocols/CODEX-NATIVE-FIRSTMATE.md`, `.agents/skills/codex-native-firstmate/SKILL.md`, `.codex/config.firstmate.example.toml`, `./{{CLI_NAME}} orchestration adapter-status --example`, and `orchestration taxonomy --example`; capability is resident per repository but activates only after a human configures a selected private instance, logical Boss contract, optional or required root materialization, task-creation grant, trust, authority, budgets, exact completion-profile coverage, presentation taxonomy, adapter, base/worktree policy, Browser/GitHub integration, heartbeat, retention/archive policy, and binding assurance; no global project registry required |
| Goal-graph workflow | inactive | `ops/protocols/GOAL-GRAPH.md`, `docs/templates/goal-graph/`, `.agents/skills/goal-graph-loop/SKILL.md`, `./{{CLI_NAME}} goals status`; compose with project orchestration when using delegated tasks |
