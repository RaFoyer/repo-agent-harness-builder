...
2. Run `./{{CLI_NAME}} lavish status` to check local posture without installing or contacting npm.
3. When using Lavish, run `./{{CLI_NAME}} lavish update --check` first if the local Lavish state is stale or unknown.
...
./{{CLI_NAME}} lavish doctor
./{{CLI_NAME}} lavish update --check
./{{CLI_NAME}} lavish update --apply
