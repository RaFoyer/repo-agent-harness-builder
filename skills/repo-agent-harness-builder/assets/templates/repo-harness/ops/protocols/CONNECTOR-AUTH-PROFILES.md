...
The repository harness should select the connector profile for every provider invocation through `./{{CLI_NAME}}` or another repo-owned wrapper. Avoid global shell exports in `.zshrc`, global "activate profile" commands, and ad hoc shared temporary files.
