...
7. Atomically bind the returned task ID only when the complete `bind` current-state contract still matches the reserved registry. Persist the emitted immutable `taskBinding` metadata with the launch key, canonical work-contract hash, node/task identity, original parent node/task binding, post-bind revision, and UTC RFC3339 bind time. For a non-Boss child, also set immutable `parentTaskId` to the immediate parent task ID used at launch; then set `state` to `working`, add `nextAction`, clear the reservation, and advance the registry revision by one. Validation recomputes the canonical contract and parentage for every task-backed node; a mismatch stays blocked until explicit supersession/replan. A failed bind must preserve the reservation and trigger reconciliation by `launchKey`, never a second create.
