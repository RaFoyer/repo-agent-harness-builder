...
- `ops/orchestration.example.json` is tracked policy/example state. It stays inactive and contains no developer identity, task ID, reservation, signature, directive, or live lifecycle state. Repository-specific tracked policy may live under its namespaced `extensions` object, which must remain free of those runtime and identity fields.
