...
Every task-backed non-Boss node records its task ID and immutable `parentTaskId` in `ops/orchestration.json`; that value must equal its immediate parent's current task ID. Titles make the hierarchy visible; immutable parent-task IDs make it durable. Do not replace a parent task while any bound child remains unreconciled.
...
7. Atomically bind the returned task ID only when the complete `bind` current-state contract still matches the reserved registry. For a non-Boss child, set immutable `parentTaskId` to the immediate parent task ID used at launch; then set `state` to `working`, add `nextAction`, clear the reservation, and advance the registry revision by one. A failed bind must preserve the reservation and trigger reconciliation by `launchKey`, never a second create.
