...
This skill name is retained for migration. Load `$goal-graph-loop` and follow
it as the authoritative workflow. A goal chain is simply a goal graph whose
nodes form one linear path; it does not need its own role, lifecycle, authority
model, or control loop.
