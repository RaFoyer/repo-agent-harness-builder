...
8. Before opening or merging feature PRs, run the discovered CLI's `no-mistakes status` command when that command exists and prefer the initialized gate.
9. Keep docs, CLI help, and tests in sync; run the CLI ergonomics audit when command output changes.
