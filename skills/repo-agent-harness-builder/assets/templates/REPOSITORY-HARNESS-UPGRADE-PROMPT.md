...
12. Audit GitHub and Git authentication before proposing migration: existing `gh`/`gh-axi` wrappers, `GH_CONFIG_DIR` and token handling, active accounts, remotes, SSH identities, credential helpers, GitHub Apps or PATs, custom PR/comment/review commands, CI bots, and repository-specific permission logic. Do not overwrite a working custom path with the generic facade.
