...
12. Audit the Codex task pin lifecycle. Treat pins as navigation, not authority or progress. The materialized resident Boss and materialized nonterminal Managers should be pinned; Workers and transient helpers must never be pinned; terminal Managers should be unpinned only after terminal completion and landed-work evidence plus parent reconciliation. Preserve ambiguously bound or quarantined Managers as pinned until reconciliation prevents duplication. Report current drift and the exact lifecycle owner for each transition.
...
- For the Codex adapter, pinning is a two-level navigation lifecycle: keep the materialized resident Boss and materialized nonterminal Managers pinned, never pin Workers or transient helpers, and unpin Managers only after terminal completion and landed-work evidence plus parent reconciliation. Pin state never grants authority or counts as progress.
