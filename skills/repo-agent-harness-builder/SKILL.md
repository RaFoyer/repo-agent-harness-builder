...
- `assets/templates/orchestration/`: inactive registry examples, ledgers, and Boss/Manager/Worker prompts with explicit trust, authority, loop ownership, evidence-fingerprint liveness, bounded retries, hybrid direct-owner messaging, optional Boss materialization, and parent reconciliation.
- `assets/templates/client-adapters/codex-native-firstmate/`: opt-in Codex config examples, role profiles, Firstmate Boss overlay, protocol, and fleet-managed adapter reference snapshot around the portable orchestration core.
...
- Git owns orchestration protocol, schema, inactive examples, CLI, and tests—not
  developer task IDs or live lifecycle. Store named operator instances under
  the Git common directory so linked worktrees share private state; use
  path-keyed private user state for non-Git folders, and never accept raw state
  path overrides.
- Keep logical hierarchy separate from client materialization. Schema-v5
  optional-root instances may launch logical-parent Managers before a Boss task
  exists; Workers still require task-backed immediate parents.
